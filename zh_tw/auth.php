...
/**
 * Strings for component 'auth', language 'zh_tw', branch 'MOODLE_22_STABLE'
 *
 * @package   auth
...
$string['auth_invalidnewemailkey'] = '錯誤：若是您正在確認 email 位置的修改，您可能從傳送給您的 email 複製 URL 的過程出了一些差錯。請複製一次位址後，再試一次。';
$string['auth_multiplehosts'] = '多個不同的主機可以被指定(例如host1.com;host2.com;host3.com)';
$string['auth_outofnewemailupdateattempts'] = '已經到達更改 email 位置許可次數的上限。您的更新請求已被取消。';
...
$string['emailchangepending'] = '更改進行中，開啟發送給 {$a->preference_newemail} 的連結。';
$string['emailnowexists'] = '由於您先前的請求，您嘗試輸入到個人資料裡的 email 位置已經指定給其他人了。因此，您的 email 的更新請求已被取消了，但您可以嘗試不同的 email 位置。';
$string['emailupdate'] = '電子郵件信箱資料更新';
$string['emailupdatemessage'] = '{$a->fullname}您好，

您在 {$a->site} 請求更改您帳號的 email 位置，請在瀏覽器開啟以下的 URL 進行確認。

{$a->url}';
$string['emailupdatesuccess'] = '使用者 <em>{$a->fullname}</em> 的 email 位置已經成功更新為 <em>{$a->email}</em> 了。';
...
$string['errormaxconsecutiveidentchars'] = '密碼必須包含最多 {$a} 個連續的相同字元。';
...
$string['informminpasswordlength'] = '至少 {$a} 個字元';
$string['informminpasswordlower'] = '至少 {$a} 個小寫字母';
$string['informminpasswordnonalphanum'] = '至少 {$a} 個非英文字母的字元';
...
$string['potentialidps'] = '用您的帳號登入在：';
$string['recaptcha'] = 'reCAPTCHA字詞驗證';
$string['recaptcha_help'] = 'CAPTCHA 是用來防止網站被自動化程式濫用，請按照順序輸入框中的文字，並以空白隔開。

若您無法確定文字為何，您可以嘗試另一個 CAPTCHA，或是音訊 CAPTCHA。';
...
$string['suspended'] = '停用的帳戶';
$string['suspended_help'] = '停用的帳戶無法登入網站及使用 web service，所有對外的訊息也將被捨棄。';
...
$string['user_activatenotsupportusertype'] = '認證：ldap user_activate() 並不支援所選的使用者類別 {$a}';
$string['user_disablenotsupportusertype'] = '認證：ldap user_disable() 並不支援選擇的使用者類別 {$a}（至少現在還不支援）';
