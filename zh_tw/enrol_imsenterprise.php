...
/**
 * Strings for component 'enrol_imsenterprise', language 'zh_tw', branch 'MOODLE_22_STABLE'
 *
...
$string['createnewcategories_desc'] = '如果<org><orgunit>出現在課程的匯入資料中，並且課程為新建，那麼它的內容會被替換為指定的分類。此插件不會對現有的課程重新分類。

如果沒有所要的名稱的類別存在，那就會建立一個隱藏的類別。';
$string['createnewcourses'] = '如果在Moodle中沒有找到，就建立一個新的（隱藏的）課程';
$string['createnewcourses_desc'] = '如果允許此項，IMS企業級插件能建立在IMS資料庫中而不在Moodle資料庫中的所有課程。所有新建的課程初始都是隱藏的。';
$string['createnewusers'] = '如果找不到就建立新的使用者';
$string['createnewusers_desc'] = 'IMS企業選課資料典型地描述一組用戶。如果啟用，會為任何在Moodle資料庫中找不到的用戶建立帳號。

首先通過"學號"查找用戶，然後再用"用戶名稱"。密碼不是通過IMS企業插件匯入的。建議使用身份認證插件驗證用戶。';
