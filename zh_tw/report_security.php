...
/**
 * Strings for component 'report_security', language 'zh_tw', branch 'MOODLE_22_STABLE'
 *
...
Email欄位也可能被認證插件所封鎖，這一可能性在此暫不考慮。';
...
$string['check_globals_details'] = '註冊全域是有高度風險的PHP設定。

必須將<code>register_globals=off</code> 放在PHP設定檔中。

你可以透過編輯你的<code>php.ini</code>，Apache/IIS，或<code>.htaccess</code> 檔案來控制它。';
$string['check_globals_error'] = '錯誤！註冊全域"必須"關閉。請立即修正這伺服器的PHP設定(在php.ini)。';
$string['check_globals_name'] = '註冊全域(Register globals)';
$string['check_globals_ok'] = '註冊全域是關閉的';
...
$string['check_noauth_details'] = '這<em>不必認證插件</em>插件不是要用於正式網站的。請關閉它，除非這是一個測試用的網站。';
$string['check_noauth_error'] = '不必認證插件不可以用在正式網站';
$string['check_noauth_name'] = '不必認證';
$string['check_noauth_ok'] = '不必認證插件是被關閉的';
...
$string['check_passwordsaltmain_details'] = '<p>設定一密碼加料可以大幅度減少密碼被破解的風險</p>
<p>要設定密碼加料，你要在你的config.php檔案中加上一行：</p>
<code>$CFG->passwordsaltmain = \' 一個很長的隨機字元的字串\';</code>
<p>這一字元的隨機字串應該包含字母、數字、和其他字元，且至少要有40個字元</p>
<p>如果你要更改密碼加料，請參考<a href="{$a}" target="_blank">密碼加料文件</a>，一旦設定，不要刪除你的密碼加料，否則密碼將全部失效，你將無法再次登入你的網站 </p>';
$string['check_passwordsaltmain_name'] = '密碼加料';
$string['check_passwordsaltmain_ok'] = '密碼加料是OK';
$string['check_passwordsaltmain_warning'] = '沒有設定密碼加料';
$string['check_passwordsaltmain_weak'] = '密碼加料是弱的';
