...
/**
 * Strings for component 'quiz', language 'es_mx', branch 'MOODLE_22_STABLE'
 *
 * @package   quiz
...
$string['configshufflequestions'] = 'Si habilita esta opción, las preguntas se ordenarán al azar cada vez que algún estudiante intenta resolver el examen.';
...
$string['decimalplaces_help'] = '<p>Mediante esta opción usted puede seleccionar el número de decimales mostrados en la calificación de cada intento.</p> Solamente afecta cómo se muestra la calificación, pero no tiene efecto sobre cómo se guarda la calificación ni en la base de datos, ni en los cálculos internos, que se realizan con precisión completa.';
...
$string['emailconfirmsubject'] = 'Confirmación de envío de examen: {$a->quizname}';
$string['emailnotifybody'] = 'Hola, {$a->username},

{$a->studentname} ha completado el examen \'{$a->quizname}\' ({$a->quizurl}) en el curso \'{$a->coursename}\'

Puede revisar este intento en {$a->quizreviewurl}.';
$string['emailnotifysmall'] = '{$a->studentname} ha completado {$a->quizname}';
$string['emailnotifysubject'] = '{$a->studentname} ha completado el examen {$a->quizname}';
$string['empty'] = 'Vacío';
...
$string['modulename_help'] = 'El módulo examen le permite al profesor diseñar y plantear exámenes con preguntas tipo opción múltiple, falso/verdadero, relacionar columnas, respuesta corta y calculada. Uy el profesor puede elegir si proporciona retroalimentación y/o se muestran las respuestas correctas.';
...
$string['reviewoptionsheading_help'] = '<p>Estas opciones controlan qué información pueden ver los usuarios cuando revisan un intento de resolver el examen o cuando consultan los correspondientes informes.</p> <p>Immediatamente después del intento: la revisión podrá hacerse en los dos minutos siguientes al final de un intento (i.e., tras haber pulsado el usuario \'Enviar todo y terminar\').</p> <p>Más tarde, mientras el examen está aún abierto: la revisión podrá hacerse en cualquier momento previo a la fecha de cierre.</p> <p>Una vez que el examen está cerrado: la revisión podrá realizarse una vez que haya pasado la fecha de cierre del examen. Obviamente, si el examen no tiene fecha de cierre, esta opción no es posible.</p> <p>A los usuarios con permiso para \'Ver calificaciones ocultas\' [moodle/grade:viewhidden] (normalmente profesores y administradores) no les afectan estas restricciones, de modo que podrán en cualquier momento revisar toda la información relativa a los intentos que los estudiantes hayan hecho de resolver el examen.</p>';
...
$string['timelimit_help'] = 'Si está activado, un temporizador se visualiza en una ventana flotante (se requiere JavaScript) mostrando una cuenta atrás. Cuando el plazo se finaliza, el examen se envía automáticamente con las respuestas realizadas hasta el momento.';
